...
const showErrorMessage = (error, t = null) => {
  ...
  // Check if this is a login error for the student account
  if (error?.config?.url?.includes('/auth/login') && 
      error?.config?.data && 
      error?.response?.status === 401) {
    try {
      const requestData = JSON.parse(error.config.data);
      if (requestData.email === 'ifimit2025@gmail.com') {
        // Dispatch custom event for student login failure
        window.dispatchEvent(new CustomEvent('showStudentAccessDialog'));
        return message; // Don't show regular toast error
      }
    } catch (e) {
