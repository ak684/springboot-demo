...
1. **Reply** to each addressed thread with a 1–2 sentence note pointing to what changed (file + line, and the commit SHA when relevant). Use the host's review-comment-reply API (GitHub: `gh api repos/{o}/{r}/pulls/{n}/comments/{id}/replies`; Bitbucket DC: `POST /rest/api/latest/projects/{k}/repos/{s}/pull-requests/{id}/comments/{commentId}/replies`).
2. **Resolve / mark "answered"** each addressed thread (GitHub: `resolveReviewThread` GraphQL mutation; Bitbucket DC: PUT the comment thread state to `RESOLVED`).
3. **Post one final summary comment** on the PR with the @-mention line. If individual reviewers are requested: `@<author> @<reviewer1> @<reviewer2> — fixes pushed, all threads resolved.` If only the author exists: `@<author> — fixes pushed, all threads resolved.`
